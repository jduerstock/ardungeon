
;		* =  $AC00
		.BYTE	6
		.WORD aToastLev4	; "toast! LEV 4"
aInA:		.BYTE "in a"
		.BYTE $A0
aRoom:		.BYTE " room"
		.BYTE $AE
aToastLev4:	.BYTE "toast! LEV 4",0  ; DATA XREF: RAM:AC01^o
		.BYTE $80
aMetallicCorrid:.BYTE "metallic corridor.",0
aAtAStairwayLea:.BYTE "at a stairway leading up.",0
aInsideASeeming:.BYTE "inside a seemingly unused room.",0
aInAnEquipmentR:.BYTE "in an equipment room.",0
		.BYTE $80
aStorageRoomOfS:.BYTE "storage room of some sort.",0
		.BYTE $80
aHiTechLoungeRo:.BYTE "hi-tech lounge room.",0
aInAnAlienContr:.BYTE "in an alien control room.",0
aInASeldomUsedA:.BYTE "in a seldom-used area.",0
aAtTheGatewayTo:.BYTE "at the Gateway to Revelation.",0
aInASmallRoom_:	.BYTE "in a small room.",0
aByTheGatewayTo:.BYTE "by the Gateway to Revelation.",0
		.BYTE $FF
aWay_:		.BYTE "way.",0
		.BYTE $80
aLongHall_:	.BYTE "long hall.",0
		.BYTE $80
aRoom_:		.BYTE "room.",0
		.BYTE $80
aSeeminglyPoint:.BYTE "seemingly pointless hall.",0
aAtTheWalk:	.BYTE "at the Walk"
		.BYTE $82
aArches_:	.BYTE "Arches.",0
aAtTheGreatDrag:.BYTE "at the Great Dragon's lair.",0
aAtASparklingFo:.BYTE "at a sparkling fountain.",0
		.BYTE	$80
aAsmall:	.BYTE "small"
		.BYTE $84
		.BYTE	0
aAtA:		.BYTE "at a"
		.BYTE $87
		.BYTE	0
aInAPassage_:	.BYTE "in a passage.",0
		.BYTE $80
aHallway_:	.BYTE "hallway.",0
aInTheHaven_:	.BYTE "in the haven.",0
aFacingAFearsom:.BYTE "facing a fearsome gargoyle.",0
		.BYTE $FF
aH_:		.BYTE "h.",0
		.BYTE $84
aUndeadKingSCry:.BYTE "Undead King's crypt.",0
		.BYTE $84
aHallOfTheAdept:.BYTE "Hall of the Adept.",0
aNearTheRiverSt:.BYTE "near the river Stonz.",0
		.BYTE $83
aBlisteringHotP:.BYTE "blistering hot passage.",0
aOnASlipperySto:.BYTE "on a slippery stone floor.",0
		.BYTE $83
aHallWithoutDoo:.BYTE "hall without doors.",0
aWalkingOnThinI:.BYTE "walking on thin ice.",0
		.BYTE $83
aSecret:	.BYTE "secret"
		.BYTE $85
		.BYTE	0
aByARiverCrossi:.BYTE "by a river crossing.",0
		.BYTE $87
		.BYTE $81
aUp_:		.BYTE "up.",0
		.BYTE $87
		.BYTE $81
aDown_:		.BYTE "down.",0
		.BYTE $84
aPaladinsGuild_:.BYTE "Paladins' Guild.",0
		.BYTE $84
aMercenariesGui:.BYTE "Mercenaries' Guild.",0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE	0
		.BYTE $20
		.BYTE $20
		.BYTE	0
		.BYTE	0
		.BYTE	7
		.BYTE  $D
		.BYTE  $B
		.BYTE  $B
		.BYTE	2
		.BYTE	8
		.BYTE  $B
		.BYTE  $B
		.BYTE  $A
		.BYTE	2
		.BYTE	8
		.BYTE  $A
		.BYTE	9
		.BYTE	9
		.BYTE	2
		.BYTE	8
		.BYTE	7
		.BYTE	9
		.BYTE	5
		.BYTE	3
		.BYTE	9
		.BYTE	3
		.BYTE  $A
		.BYTE	2
		.BYTE	3
		.BYTE	9
		.BYTE	7
		.BYTE  $B
		.BYTE	3
		.BYTE	3
		.BYTE  $B
		.BYTE	8
		.BYTE  $E
		.BYTE	2
		.BYTE	3
		.BYTE  $E
		.BYTE	6
		.BYTE  $F
		.BYTE	4
		.BYTE	3
		.BYTE  $C
		.BYTE $10
		.BYTE $10
		.BYTE  $D
		.BYTE	4
		.BYTE	2
		.BYTE  $F
		.BYTE	5
		.BYTE  $D
		.BYTE	5
		.BYTE  $B
		.BYTE	2
		.BYTE  $D
		.BYTE	0
		.BYTE	6
		.BYTE $11
		.BYTE  $E
		.BYTE $15
		.BYTE	5
		.BYTE	9
		.BYTE $19
		.BYTE $1F
		.BYTE $1C
		.BYTE $1D
		.BYTE  $B
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	0
		.BYTE $FF
		.BYTE	6
		.BYTE $8E ; Ž
		.BYTE	3
		.BYTE $88 ; ˆ
		.BYTE  $E
		.BYTE $7C ; |
		.BYTE  $C
		.BYTE	8
		.BYTE	1
		.BYTE $F5 ; õ
		.BYTE	0
		.BYTE $24 ; $
		.BYTE	0
		.BYTE  $A
		.BYTE $34 ; 4
		.BYTE $28 ; (
		.BYTE	1
		.BYTE $80 ; €
		.BYTE	2
		.BYTE $24 ; $
		.BYTE	0
		.BYTE  $A
		.BYTE	6
		.BYTE	2
		.BYTE	3
		.BYTE $F5 ; õ
		.BYTE	0
		.BYTE $34 ; 4
		.BYTE	0
		.BYTE $1A
		.BYTE $34 ; 4
		.BYTE	2
		.BYTE	1
		.BYTE $64 ; d
		.BYTE	0
		.BYTE  $A
		.BYTE	0
		.BYTE	4
		.BYTE  $E
		.BYTE $24 ; $
		.BYTE	1
		.BYTE $86 ; †
		.BYTE	0
		.BYTE $24 ; $
		.BYTE	0
		.BYTE  $A
		.BYTE $24 ; $
		.BYTE $C4 ; Ä
		.BYTE	1
		.BYTE $87 ; ‡
		.BYTE	0
		.BYTE $24 ; $
		.BYTE	0
		.BYTE  $A
		.BYTE $7A ; z
		.BYTE $7A ; z
		.BYTE $32 ; 2
		.BYTE	5
		.BYTE $80 ; €
		.BYTE	0
		.BYTE $44 ; D
		.BYTE $86 ; †
		.BYTE $1A
		.BYTE $34 ; 4
		.BYTE $54 ; T
		.BYTE	5
		.BYTE $45 ; E
		.BYTE	0
		.BYTE $44 ; D
		.BYTE	0
		.BYTE $28 ; (
		.BYTE $34 ; 4
		.BYTE $54 ; T
		.BYTE	1
		.BYTE $89 ; ‰
		.BYTE	0
		.BYTE $22 ; "
		.BYTE	0
		.BYTE  $A
		.BYTE	4
		.BYTE	6
		.BYTE $74 ; t
		.BYTE	0
		.BYTE $86 ; †
		.BYTE $C4 ; Ä
		.BYTE $CA ; Ê
		.BYTE $98 ; ˜
		.BYTE $86 ; †
		.BYTE	3
		.BYTE $A5 ; ¥
		.BYTE	0
		.BYTE $34 ; 4
		.BYTE	0
		.BYTE $1A
		.BYTE $1A
		.BYTE $34 ; 4
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE  $D
		.BYTE $D0 ; Ð
		.BYTE	1
		.BYTE	0
		.BYTE  $D
		.BYTE  $D
		.BYTE	1
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE	0
		.BYTE	1
		.BYTE	0
		.BYTE $3D ; =
		.BYTE $D1 ; Ñ
		.BYTE  $B
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE $3D ; =
		.BYTE	9
		.BYTE	4
		.BYTE  $D
		.BYTE $D0 ; Ð
		.BYTE	8
		.BYTE	0
		.BYTE  $D
		.BYTE  $D
		.BYTE	8
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE	1
		.BYTE	8
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE $D0 ; Ð
		.BYTE	1
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE $D0 ; Ð
		.BYTE	2
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE $D3 ; Ó
		.BYTE	1
		.BYTE	0
		.BYTE	1
		.BYTE $30 ; 0
		.BYTE	8
		.BYTE	0
		.BYTE  $D
		.BYTE	0
		.BYTE	8
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE  $D
		.BYTE	8
		.BYTE	0
		.BYTE  $D
		.BYTE $D3 ; Ó
		.BYTE	8
		.BYTE	0
		.BYTE $D1 ; Ñ
		.BYTE  $D
		.BYTE	8
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE $D0 ; Ð
		.BYTE	1
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE $DD ; Ý
		.BYTE	2
		.BYTE	0
		.BYTE $D3 ; Ó
		.BYTE $DD ; Ý
		.BYTE  $A
		.BYTE $1C
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE	8
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE  $D
		.BYTE	8
		.BYTE	0
		.BYTE  $D
		.BYTE $D0 ; Ð
		.BYTE	7
		.BYTE $1B
		.BYTE $D3 ; Ó
		.BYTE	0
		.BYTE	7
		.BYTE $1B
		.BYTE $DD ; Ý
		.BYTE $DD ; Ý
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE $D0 ; Ð
		.BYTE	1
		.BYTE	0
		.BYTE  $D
		.BYTE $D0 ; Ð
		.BYTE	4
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE	0
		.BYTE	4
		.BYTE	0
		.BYTE  $D
		.BYTE $D0 ; Ð
		.BYTE	4
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE	0
		.BYTE	4
		.BYTE	0
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE	7
		.BYTE $1B
		.BYTE $30 ; 0
		.BYTE  $D
		.BYTE	7
		.BYTE $1B
		.BYTE $DD ; Ý
		.BYTE $31 ; 1
		.BYTE	1
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE $D0 ; Ð
		.BYTE	1
		.BYTE	0
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE	4
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE	0
		.BYTE	4
		.BYTE	0
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE	4
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE	0
		.BYTE	4
		.BYTE	0
		.BYTE  $D
		.BYTE $D0 ; Ð
		.BYTE	1
		.BYTE	0
		.BYTE  $D
		.BYTE  $D
		.BYTE	1
		.BYTE	0
		.BYTE $D1 ; Ñ
		.BYTE  $D
		.BYTE	1
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE $D1 ; Ñ
		.BYTE	1
		.BYTE	0
		.BYTE	0
		.BYTE $D3 ; Ó
		.BYTE	4
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE  $D
		.BYTE	4
		.BYTE	0
		.BYTE	0
		.BYTE $D3 ; Ó
		.BYTE	4
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE  $D
		.BYTE	4
		.BYTE	0
		.BYTE $30 ; 0
		.BYTE $D1 ; Ñ
		.BYTE	1
		.BYTE	0
		.BYTE  $D
		.BYTE $30 ; 0
		.BYTE	6
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE	0
		.BYTE	6
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	1
		.BYTE $DD ; Ý
		.BYTE	1
		.BYTE	0
		.BYTE	3
		.BYTE	3
		.BYTE	1
		.BYTE	0
		.BYTE  $D
		.BYTE  $D
		.BYTE	1
		.BYTE	0
		.BYTE	3
		.BYTE	3
		.BYTE	1
		.BYTE	0
		.BYTE  $D
		.BYTE  $D
		.BYTE	1
		.BYTE	0
		.BYTE $D1 ; Ñ
		.BYTE	3
		.BYTE	1
		.BYTE	0
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE	6
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE	0
		.BYTE	6
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE	0
		.BYTE  $D
		.BYTE $DD ; Ý
		.BYTE	5
		.BYTE	0
		.BYTE $D3 ; Ó
		.BYTE  $D
		.BYTE	5
		.BYTE	0
		.BYTE  $D
		.BYTE $DD ; Ý
		.BYTE	3
		.BYTE $BF ; ¿
		.BYTE $D3 ; Ó
		.BYTE  $D
		.BYTE	3
		.BYTE	0
		.BYTE  $D
		.BYTE $DD ; Ý
		.BYTE	5
		.BYTE	0
		.BYTE $D3 ; Ó
		.BYTE  $D
		.BYTE	5
		.BYTE	0
		.BYTE	0
		.BYTE $DD ; Ý
		.BYTE	6
		.BYTE	0
		.BYTE $D0 ; Ð
		.BYTE  $D
		.BYTE	6
		.RES	664,$00
		.BYTE $FF
		.RES	2504,$00
; end of 'RAM'


		.END
